...
Predictors are built through the production resolver (`_build_named_ml_predictor`)
per test so each can run in its own process — the native ML backends
(xgboost/lightgbm/catboost/torch) can crash if many load in one interpreter (see the
KMP_DUPLICATE_LIB_OK note in the dev docs). The six sklearn-portfolio models share one
class but keep distinct names; the metadata contract below is unchanged by that collapse.
"""
...
def _build(name: str):
    """Construct a predictor by canonical name via the production resolver."""
    return _build_named_ml_predictor(name)


# (predictor-name, one metadata key it must emit)
_MODELS = [
    ("catboost", "iterations"),
...
@pytest.mark.parametrize("name, meta_key", _MODELS, ids=_IDS)
...
@pytest.mark.parametrize("name", _IDS + ["deep_learning"], ids=_IDS + ["deep_learning"])

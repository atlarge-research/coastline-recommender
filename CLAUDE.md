...
- **The parametric ML models ship in the wheel; the large ones do not.** Five models (catboost, xgboost, lightgbm, bayesian_ridge, deep_learning) are bundled under `src/coastline/sdk/predictors/performance/data_driven/portfolio/` and ship in git + the wheel + Docker. Repo-root `models/` is split: `models/coastline-bundled/` holds the shipped portfolio (the large/instance-based pickles — tabpfn, random_forest via Git LFS, knn, gaussian_process, svr — plus symlinks to the packaged five), `models/custom/` holds user-tuned artifacts from `coastline tune`. Resolution precedence: `custom/` > `coastline-bundled/` > flat `PORTFOLIO_DIR` > packaged portfolio. The default Kavier physics path needs no pickles.

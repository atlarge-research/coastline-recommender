"""KNN predictor (#3 in portfolio): featv3 pickle, log1p target, LabelEncoder categoricals."""
...
_MODEL_PATH = performance_trained_model_path("knn")


class KNNPredictor(BasePredictor):
    """KNN predictor for dataset_tokens_per_second."""

    def __init__(self, model_path: Optional[Path] = None):
        self._model_path = model_path or _MODEL_PATH
        self._model = None  # Pipeline with scaler + KNN
        self._encoders = None
    ...
    def _load(self):
        ...
        if not self._model_path.exists():
            raise FileNotFoundError(
                f"KNN model not found at {self._model_path}. Train it first: python -m trainer.main --model knn"
            )
        ...
        try:
            ...
            logger.info(f"KNN model loaded from {self._model_path}")
            ...
            if self._best_params:
                logger.info(
                    f"  Best params: n_neighbors={self._best_params.get('knn__n_neighbors')}, "
                    f"weights={self._best_params.get('knn__weights')}, "
                    f"p={self._best_params.get('knn__p')}"
                )
    ...
    def predict(self, workload: WorkloadSpec, context: SystemContext) -> Optional[Prediction]:
        ...
        try:
            ...
            logger.warning(f"KNN predictor unavailable: {e}")
            return None
        ...
        if model is None or encoders is None or cat_features is None or num_features is None:
            logger.warning("KNN predictor artifacts are incomplete")
            return None

        row = workload_to_ml_feature_row(workload)
        if feature_row_has_unknown_specs(row):
            logger.info("KNN: unknown model/GPU specs, cannot predict")
            return None
        ...
        return finalize_ml_prediction(
            ...
            metadata={
                "predictor": "knn",
                "n_neighbors": best_params.get("knn__n_neighbors", "N/A"),
                "weights": best_params.get("knn__weights", "N/A"),
                "p": best_params.get("knn__p", "N/A"),
                "metric": "minkowski",
                "cache_hit": False,

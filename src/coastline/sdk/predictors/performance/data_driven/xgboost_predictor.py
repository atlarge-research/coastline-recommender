"""XGBoost predictor (#6 in portfolio): featv3 pickle, log1p target, LabelEncoder categoricals."""
...
_MODEL_PATH = performance_trained_model_path("xgboost")


class XGBoostPredictor(BasePredictor):
    """XGBoost predictor for dataset_tokens_per_second."""
    ...
    def _load(self):
        ...
        if not self._model_path.exists():
            raise FileNotFoundError(
                f"XGBoost model not found at {self._model_path}. Train it first: python -m trainer.main --model xgboost"
            )
    ...
    def predict(self, workload: WorkloadSpec, context: SystemContext) -> Optional[Prediction]:
        ...
        try:
            ...
            logger.warning(f"XGBoost unavailable, skipping prediction: {e}")
            return None
        ...
        if model is None or encoders is None or cat_features is None or num_features is None:
            logger.warning("XGBoost predictor artifacts are incomplete")
            return None

        row = workload_to_ml_feature_row(workload)
        if feature_row_has_unknown_specs(row):
            logger.info("xgboost: unknown model/GPU specs, cannot predict")
            return None
